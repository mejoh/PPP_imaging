...
# Load clinical data
fClin <- '/Volumes/project/3022026.01/pep/ClinVars_10-08-2023/derivatives/merged_manipulated_2024-07-17.csv'
...
clinvars <- c('pseudonym', 'TimepointNr', 'YearsToFollowUp', 'ParticipantType',
              ...
              'MotComFluctImpact','MotComFluctComplex')
...
# Motor fluctuations
cat('>>> Motor fluctuations: \n')
dfClin %>% filter(ParticipantType=='PD_POM', TimepointNr %in% c(0,2)) %>% select(TimepointNr, MotComFluctImpact) %>% group_by(TimepointNr) %>% table() %>% print()
dfClin %>% filter(ParticipantType=='PD_POM', TimepointNr %in% c(0,2)) %>% select(TimepointNr, MotComFluctComplex) %>% group_by(TimepointNr) %>% table() %>% print()
